...
namespace NakhleNakhoda.Data.Core
{
    ...
    public partial class CommonHelper
    {
        ...
        {
            var randomNumberBuffer = RandomNumberGenerator.GetBytes(10);
        ...
        /// <summary>
        /// Converts a value to a Location type.
        /// </summary>
        /// <param name="value">The value to convert.</param>
        /// <param name="LocationType">The type to convert the value to.</param>
        /// <returns>The converted value.</returns>
        public static object? To(object value, Type LocationType)
        {
            return To(value, LocationType, CultureInfo.InvariantCulture);
        }
        ...
        /// <returns>The converted value.</returns>
        public static object? To(object value, Type LocationType, CultureInfo culture)
        {
            if (value == null)
                return null;
            ...
            var LocationConverter = TypeDescriptor.GetConverter(LocationType);
            if (LocationConverter.CanConvertFrom(value.GetType()))
                return LocationConverter.ConvertFrom(null, culture, value);

            var sourceConverter = TypeDescriptor.GetConverter(sourceType);
            if (sourceConverter.CanConvertTo(LocationType))
                return sourceConverter.ConvertTo(null, culture, value, LocationType);

            if (LocationType.IsEnum && value is int)
                return Enum.ToObject(LocationType, (int)value);

            if (!LocationType.IsInstanceOfType(value))
                return Convert.ChangeType(value, LocationType, culture);
