...
namespace NakhleNakhoda.Data.Core.Infrastructure.Mapper.Extensions
{
    /// <summary>
    ...
    public static class MappingExtensions
    {
        ...
        /// <summary>
        /// Execute a mapping from the source queryable to a new destination queryable. The source type is inferred from the source queryable
        /// </summary>
        /// <typeparam name="TDestination">Destination object type</typeparam>
        /// <param name="source">Source object to map from</param>
        /// <returns>Mapped destination query</returns>
        private static IQueryable<TDestination> ProjectTo<TDestination>(this IQueryable source)
        {
            //use AutoMapper for mapping objects
            return AutoMapperConfiguration.Mapper!.ProjectTo<TDestination>(source);
        }
        ...
        /// <param name="entity">Entity to map from</param>
        /// <returns>Mapped model</returns>
        public static IQueryable<TModel> ToModel<TModel>(this IQueryable queryable) where TModel : class
        {
            if (queryable == null)
                throw new ArgumentNullException(nameof(queryable));

            return queryable.ProjectTo<TModel>();
        }
        ...
        /// <typeparam name="TEntity">Entity type</typeparam>
        /// <typeparam name="TModel">Model type</typeparam>
        /// <param name="entities">Entities to map from</param>
        /// <param name="model">Model to map into</param>
        /// <returns>Mapped model</returns>
        public static List<TModel> ToModel<TEntity, TModel>(this IList<TEntity> entities)
            where TEntity : BaseEntity where TModel : BaseEntityModel
        {
            if (entities == null)
                throw new ArgumentNullException(nameof(entities));

            return entities.Map<List<TModel>>();
        }
