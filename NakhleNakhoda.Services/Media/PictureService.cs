...
namespace NakhleNakhoda.Services.Media
{
    ...
    /// Picture service
    /// </summary>
    public partial class PictureService : Repository<Picture>, IPictureService
    {
        #region Fields
        ...
        private readonly IPictureBinaryService _pictureBinaryRepository;
        ...
        public PictureService(
            ...
            IPictureBinaryService pictureBinaryRepository) : base(uow)
        {
        ...
        protected virtual string ToUnichar(string hexString)
        {
            ...
            b[1] = Convert.ToByte(hexString[..2], 16);
        ...
        public virtual byte[] GetPictureBits(IFormFile file)
        {
...
*/
        ...
        protected virtual byte[] LoadPictureFromFile(long pictureId, string mimeType)
        {
            ...
            var fileName = $"{pictureId}-0.{lastPart}";
        ...
        protected virtual void DeletePictureOnFileSystem(Picture picture)
        {
            ...
            var fileName = $"{picture.Id}-0.{lastPart}";
            var filePath = GetPictureLocalPath(fileName);
        ...
        protected virtual string GetThumbLocalPath(string thumbFileName)
        {
            ...
            if (MediaDefaults.MultipleThumbDirectories)
            {
                ...
                {
                    var subDirectoryName = fileNameWithoutExtension[..MediaDefaults.MultipleThumbDirectoriesLength];
        ...
        protected virtual async Task<byte[]> LoadPictureBinary(Picture picture, bool fromDb)
        {
            ...
            var result = fromDb
                ? pictureBinary?.BinaryData ?? Array.Empty<byte>()
        ...
        protected virtual Task SaveThumb(string thumbFilePath, string thumbFileName, string mimeType, byte[] binary)
        {
        ...
        {
            string defaultImageFileName = defaultPictureType switch
            {
                PictureType.Avatar => MediaDefaults.DefaultAvatarFileName,//_settingService.GetSettingByKey("Media.Customer.DefaultAvatarImageName", MediaDefaults.DefaultAvatarFileName);
                _ => MediaDefaults.DefaultImageFileName,//_settingService.GetSettingByKey("Media.DefaultImageName", MediaDefaults.DefaultImageFileName);
            };


            /*var defaultImageFileName = defaultPictureType switch
            {
                PictureType.Avatar => _settingService.GetSettingByKey("Media.Customer.DefaultAvatarImageName", MediaDefaults.DefaultAvatarFileName),
                _ => _settingService.GetSettingByKey("Media.DefaultImageName", MediaDefaults.DefaultImageFileName),
            };*/
            ...
            else
            {
                ...
                if (!await GeneratedThumbExists(thumbFilePath, thumbFileName))
                {
                    ...
                    await SaveThumb(thumbFilePath, thumbFileName, imageFormat.DefaultMimeType, pictureBinary);
                }
        ...
        /// <param name="defaultPictureType">Default picture type</param>
        /// <returns>Picture URL</returns>
        public virtual async Task<string> GetPictureUrl(long pictureId,
            int targetSize = 0,
            ...
            PictureType defaultPictureType = PictureType.Entity)
        {
            ...
            if (targetSize == 0)
            {
                thumbFileName = !string.IsNullOrEmpty(seoFileName)
                    ? $"{seoFileName}-{picture.Id}.{lastPart}"
                    : $"{picture.Id}.{lastPart}";
                ...
                await SaveThumb(thumbFilePath, thumbFileName, picture.MimeType, pictureBinaryResized);
            ...
            {
                thumbFileName = !string.IsNullOrEmpty(seoFileName)
                    ? $"{seoFileName}-{picture.Id}-{targetSize}.{lastPart}"
                    : $"{targetSize}-{picture.Id}.{lastPart}";
        ...
        public virtual async Task<Picture?> GetPictureById(long pictureId)
        {
            if (pictureId == 0)
                return null;

            return await GetByIdAsync(pictureId);
        }
        ...
        public virtual async Task<Picture?> InsertPicture(IFormFile formFile, string defaultFileName = "", string virtualPath = "")
        {
            ...
            var picture = new Picture();
            picture = await InsertPicture(GetPictureBits(formFile), contentType, fileName, fileName, fileName);
        ...
        /// <param name="validateBinary">A value indicating whether to validated provided picture binary</param>
        /// <returns>Picture</returns>
        public virtual async Task<Picture?> UpdatePicture(long pictureId, byte[] pictureBinary, string mimeType,
        ...
        public virtual async Task<Picture?> UpdatePicture(Picture picture)
        {
            ...
            await UpdateAsync(picture);
            var pictureBinary = await GetPictureBinaryByPictureId(picture.Id);
            if (pictureBinary?.BinaryData == null) return null;

            await UpdatePictureBinary(picture, StoreInDb ? pictureBinary.BinaryData : Array.Empty<byte>());

            if (!StoreInDb)
                SavePictureInFile(picture.Id, pictureBinary.BinaryData, picture.MimeType);
        ...
        /// <param name="pictureId">The picture identifier</param>
        /// <returns>Picture binary</returns>
        public virtual async Task<PictureBinary?> GetPictureBinaryByPictureId(long pictureId)
        {
        ...
        /// <param name="seoFilename">The SEO filename</param>
        /// <returns>Picture</returns>
        public virtual async Task<Picture?> SetSeoFilename(long pictureId, string seoFilename,
            string? altAttribute = null, string? titleAttribute = null)
        {
            var picture = await GetPictureById(pictureId) ?? throw new ArgumentException("No picture found with the specified id");
        ...
        public virtual byte[] ValidatePicture(byte[] pictureBinary, string mimeType)
        {
            ...
            if (Math.Max(image.Height, image.Width) > MediaDefaults.MaximumImageSize)
            {
                image.Mutate(imageProcess => imageProcess.Resize(new ResizeOptions
                {
                    Mode = ResizeMode.Max,
                    Size = new Size(MediaDefaults.MaximumImageSize)
                }));
            }
