...
namespace NakhleNakhoda.Services.Media.RoxyFileman
{
    ...
    public class DatabaseRoxyFilemanService : FileRoxyFilemanService
    {
        ...
        public DatabaseRoxyFilemanService(IPictureService pictureService,
            ...
            ICustomFileProvider fileProvider) : base(webHostEnvironment, httpContextAccessor, fileProvider)
        {
        ...
        protected virtual Picture? GetPictureByFile(string filePath)
        {
            ...
            var sourceVirtualPath = _fileProvider.GetVirtualPath(dir);
        ...
        /// <param name="LocationPath">Path to the Location directory</param>
        protected override async Task CopyDirectory(string sourcePath, string LocationPath)
        {
            var pictures = _pictureService.GetPictures($"{_fileProvider.GetVirtualPath(sourcePath).TrimEnd('/')}/");
            var baseLocationPathVirtualPath = _fileProvider.GetVirtualPath(LocationPath);

            foreach (var picture in pictures)
            {
                var LocationPathVirtualPath =
                    $"{baseLocationPathVirtualPath.TrimEnd('/')}{picture!.VirtualPath.Replace(_fileProvider.GetVirtualPath(sourcePath), "")}";
                var pictureBinary = await _pictureService.GetPictureBinaryByPictureId(picture.Id);
                var mimeType = _pictureService.GetFileExtensionFromMimeType(picture.MimeType);
                if (pictureBinary != null && mimeType != null)
                {
                    await _pictureService.InsertPicture(new RoxyFilemanFormFile(picture, pictureBinary, mimeType), string.Empty, LocationPathVirtualPath);
                }
            }
        }
            ...
            where TPixel : unmanaged, IPixel<TPixel>
        {
            ...
            switch (imageEncoder)
            {
                case JpegEncoder jpegEncoder:
                    jpegEncoder.ColorType = JpegColorType.YCbCrRatio444;
                    jpegEncoder.Quality = quality ?? MediaDefaults.DefaultImageQuality;
        ...
        {
            var pictureBinary = await _pictureService.GetPictureBinaryByPictureId(picture.Id);
            if (pictureBinary == null) return;
            var image = Image.Load(pictureBinary.BinaryData);
        ...
        public override async Task MoveDirectoryAsync(string sourcePath, string LocationPath)
        {
            await base.MoveDirectoryAsync(sourcePath, LocationPath);
            ...
            foreach (var picture in pictures)
            {
                ...
                var LocationPathVirtualPath =
                    $"{baseLocationPathVirtualPath.TrimEnd('/')}/{_fileProvider.GetDirectoryNameOnly(_fileProvider.GetAbsolutePath(sourcePath.TrimStart('~').Split('/')))}";

                picture.VirtualPath = LocationPathVirtualPath;
        ...
        public override async Task RenameDirectoryAsync(string sourcePath, string newName)
        {
            ...
            foreach (var picture in pictures)
            {
                ...
                picture.VirtualPath = LocationPath;
        ...
        /// <param name="LocationPath">Path to the Location file</param>
        /// <returns>A task that represents the completion of the operation</returns>
        public override async Task CopyFileAsync(string sourcePath, string LocationPath)
        {
            var filePath = _fileProvider.GetAbsolutePath(sourcePath.Split('/'));
            var picture = GetPictureByFile(filePath) ?? throw new Exception(GetLanguageResource("E_CopyFile"));

            var pictureBinary = await _pictureService.GetPictureBinaryByPictureId(picture.Id);
            if (pictureBinary == null) return;

            await _pictureService.InsertPicture(
                new RoxyFilemanFormFile(picture, pictureBinary, _fileProvider.GetFileExtension(filePath)),
                string.Empty, _fileProvider.GetVirtualPath(LocationPath));
        ...
        {
            var filePath = _fileProvider.GetAbsolutePath(sourcePath.Split('/'));
            var picture = GetPictureByFile(filePath) ?? throw new Exception(GetLanguageResource("E_DeletеFile"));
        ...
        /// <param name="LocationPath">Path to the Location file</param>
        /// <returns>A task that represents the completion of the operation</returns>
        public override async Task MoveFileAsync(string sourcePath, string LocationPath)
        {
            await base.MoveFileAsync(sourcePath, LocationPath);

            var filePath = _fileProvider.GetAbsolutePath(sourcePath.Split('/'));
            var picture = GetPictureByFile(filePath) ?? throw new Exception(GetLanguageResource("E_MoveFile"));
            picture.VirtualPath = _fileProvider.GetVirtualPath(_fileProvider.GetVirtualPath(_fileProvider.GetDirectoryName(LocationPath)!));
            await _pictureService.UpdatePicture(picture);
        }
        ...
        public override async Task RenameFileAsync(string sourcePath, string newName)
        {
            ...
            var picture = GetPictureByFile(filePath) ?? throw new Exception(GetLanguageResource("E_RenameFile"));
            var fileName = _fileProvider.GetFileNameWithoutExtension(newName);
            ...
            try
            {
                ...
                foreach (var formFile in GetHttpContext().Request.Form.Files)
                {
                    ...
                    if (CanHandleFile(fileName))
                    {
                        ...
                        var LocationFile = _fileProvider.Combine(fullPath, uniqueFileName);
                        ...
                        {
                            using var stream = new FileStream(LocationFile, FileMode.OpenOrCreate);
                            formFile.CopyTo(stream);
                        }
                        else
                        {
                            await _pictureService.InsertPicture(formFile, virtualPath: GetVirtualPath(directoryPath));
                        }
                    }
                    else
                    {
        ...
        public override void FlushAllImagesOnDisk(bool removeOriginal = true)
        {
            ...
            _ = int.TryParse(GetSetting("MAX_IMAGE_WIDTH"), out var width);
            _ = int.TryParse(GetSetting("MAX_IMAGE_HEIGHT"), out var height);
            ...
            try
            {
                while (true)
                {
                    ...
                    foreach (var picture in pictures)
                    {
                        FlushImages(picture!, width, height).Wait();
                    }
