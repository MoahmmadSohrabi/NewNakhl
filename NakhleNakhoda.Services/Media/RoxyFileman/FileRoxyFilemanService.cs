...
namespace NakhleNakhoda.Services.Media.RoxyFileman
{
    ...
    public class FileRoxyFilemanService : BaseRoxyFilemanService, IRoxyFilemanService
    {
        ...
        protected virtual async Task CopyDirectory(string sourcePath, string LocationPath)
        {
            var existingFiles = _fileProvider.GetFiles(sourcePath);
            var existingDirectories = _fileProvider.GetDirectories(sourcePath);

            if (!_fileProvider.DirectoryExists(LocationPath))
                _fileProvider.CreateDirectory(LocationPath);

            foreach (var file in existingFiles)
            {
                var filePath = _fileProvider.Combine(LocationPath, _fileProvider.GetFileName(file));
            ...
            {
                var directoryPath = _fileProvider.Combine(LocationPath, _fileProvider.GetDirectoryName(directory)!);
                await CopyDirectory(directory, directoryPath);
            }
        }
        ...
        /// <param name="destinstionPath">Path to the Location image</param>
        ...
        public virtual async Task CopyDirectoryAsync(string sourcePath, string LocationPath)
        {
            ...
            var newDirectoryPath = GetFullPath(GetVirtualPath($"{LocationPath.TrimEnd('/')}/{_fileProvider.GetDirectoryNameOnly(directoryPath)}"));
            ...
            await CopyDirectory(directoryPath, newDirectoryPath);
        ...
        {
            var rootDirectoryPath = GetFullPath(GetVirtualPath(""));
        ...
        /// <param name="LocationPath">Path to the Location directory</param>
        /// <returns>A task that represents the completion of the operation</returns>
        public virtual async Task MoveDirectoryAsync(string sourcePath, string LocationPath)
        {
            var fullSourcePath = GetFullPath(GetVirtualPath(sourcePath));

            LocationPath = GetFullPath(GetVirtualPath(_fileProvider.Combine(LocationPath, _fileProvider.GetDirectoryNameOnly(fullSourcePath))));

            if (LocationPath.IndexOf(fullSourcePath, StringComparison.InvariantCulture) == 0)
            ...
            if (!IsPathAllowed(fullSourcePath) || !IsPathAllowed(LocationPath))
                throw new Exception(GetLanguageResource("E_MoveDirInvalisPath"));

            try
            {
                _fileProvider.DirectoryMove(fullSourcePath, LocationPath);
                await GetHttpContext().Response.WriteAsync(GetSuccessResponse());
            }
            catch
            {
        ...
        public virtual async Task RenameDirectoryAsync(string sourcePath, string newName)
        {
            ...
            var LocationDirectory = _fileProvider.Combine(_fileProvider.GetParentDirectory(fullSourcePath), newName);
            ...
            if (_fileProvider.DirectoryExists(LocationDirectory))
                throw new Exception(GetLanguageResource("E_DirAlreadyExists"));

            if (!IsPathAllowed(LocationDirectory))
                throw new Exception(GetLanguageResource("E_RenameDirInvalidPath"));

            try
            {
                _fileProvider.DirectoryMove(fullSourcePath, LocationDirectory);
                await GetHttpContext().Response.WriteAsync(GetSuccessResponse());
            }
            catch
            {
        ...
        /// <returns>A task that represents the completion of the operation</returns>
        public virtual async Task CopyFileAsync(string sourcePath, string LocationPath)
        {
            ...
            LocationPath = GetFullPath(GetVirtualPath(LocationPath));

            if (!IsPathAllowed(filePath) || !IsPathAllowed(LocationPath))
                throw new Exception(GetLanguageResource("E_CopyFileInvalisPath"));

            var newFileName = GetUniqueFileName(LocationPath, _fileProvider.GetFileName(filePath));
            try
            {
                _fileProvider.FileCopy(filePath, _fileProvider.Combine(LocationPath, newFileName));
                await GetHttpContext().Response.WriteAsync(GetSuccessResponse());
            }
            catch
            {
        ...
        /// <param name="LocationPath">Path to the Location file</param>
        /// <returns>A task that represents the completion of the operation</returns>
        public virtual async Task MoveFileAsync(string sourcePath, string LocationPath)
        {
            var fullSourcePath = GetFullPath(GetVirtualPath(sourcePath));
            ...
            if (_fileProvider.FileExists(LocationPath))
                throw new Exception(GetLanguageResource("E_MoveFileAlreadyExists"));

            if (!CanHandleFile(_fileProvider.GetFileName(LocationPath)))
            ...
            try
            {
                _fileProvider.FileMove(fullSourcePath, LocationPath);
                await GetHttpContext().Response.WriteAsync(GetSuccessResponse());
            }
            catch
            {
        ...
        public virtual async Task RenameFileAsync(string sourcePath, string newName)
        {
            ...
            var LocationPath = _fileProvider.Combine(_fileProvider.GetDirectoryName(fullSourcePath)!, newName);
            ...
            try
            {
                foreach (var formFile in GetHttpContext().Request.Form.Files)
                {
                    ...
                    if (CanHandleFile(fileName))
                    {
                        ...
                        var LocationFile = _fileProvider.Combine(directoryPath, uniqueFileName);
                        ...
                        ImageResize(LocationFile, LocationFile, w, h);
                    }
                    else
                    {
                        hasErrors = true;
