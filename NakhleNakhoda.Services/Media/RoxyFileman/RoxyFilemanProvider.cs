...
namespace NakhleNakhoda.Services.Media.RoxyFileman
{
    ...
    public class RoxyFilemanProvider : IFileProvider
    {
        ...
        {
            //var pictureService = Singleton<IEngine>.Instance.Resolve<IPictureService>();
            bool StoreInDb = true;
            if (_physicalFileProvider.GetFileInfo(subpath).Exists || !StoreInDb)
                return _physicalFileProvider.GetFileInfo(subpath);

            var fileProvider = Singleton<IEngine>.Instance.Resolve<ICustomFileProvider>() as CustomFileProvider;
            var roxyFilemanService = Singleton<IEngine>.Instance.Resolve<IRoxyFilemanService>();
