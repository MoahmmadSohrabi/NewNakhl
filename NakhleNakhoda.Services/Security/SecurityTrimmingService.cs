...
namespace NakhleNakhoda.Services.Security
{
    public class SecurityTrimmingService : ISecurityTrimmingService
    {
        ...
        public bool CanUserAccess(ClaimsPrincipal user, string area, string controller, string action)
        {
            ...
            {
                throw new KeyNotFoundException($"The `secured` area={area}/controller={controller}/action={action} with `PolicieNames.DynamicPermission` policy not found. Please check you have entered the area/controller/action names correctly and also it's decorated with the correct security policy.");
            }*/
            ...
            {
                // Developer and Admin users have access to all of the pages.
                return true;
            }
            ...
            var userHasClaim = user.HasClaim(claim =>
                string.Equals(claim.Type, PolicieNames.DynamicPermissionClaimType, StringComparison.InvariantCultureIgnoreCase) &&
                string.Equals(claim.Value, currentClaimValue, StringComparison.InvariantCultureIgnoreCase));
