...
using NakhleNakhoda.Web.Framework.Models;

namespace NakhleNakhoda.Web.Framework.TagHelpers.Site
{
    /// <summary>
    /// site-input tag helper
    /// </summary>
    [HtmlTargetElement("site-input", Attributes = ForAttributeName, TagStructure = TagStructure.WithoutEndTag)]
    public class SiteInputTagHelper : TagHelper
    {
        ...
        public SiteInputTagHelper(IHtmlHelper htmlHelper)
        {
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            //disabled attribute
            _ = bool.TryParse(IsDisabled, out var disabled);
            if (disabled)
            {
            ...
            //readonly attribute
            _ = bool.TryParse(IsReadonly, out var Readonly);
            if (Readonly)
            {
            ...
            //required asterisk
            _ = bool.TryParse(IsRequired, out var required);
            if (required)
            {
            ...
            if (string.IsNullOrEmpty(RenderFormControlClass) && For!.Metadata.ModelType.Name.Equals("String") || renderFormControlClass)
                htmlAttributes.Add("class", $"form-control {Class}");
