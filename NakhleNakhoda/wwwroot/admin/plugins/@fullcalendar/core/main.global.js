    ...
    */
    ...
    var SegHierarchy = /** @class */ (function () {
        ...
        SegHierarchy.prototype.findInsertion = function (newEntry) {
            ...
            var stackCnt = 0;
            for (var NakhleNakhodaLevel = 0; NakhleNakhodaLevel < levelCnt; NakhleNakhodaLevel += 1) {
                var NakhleNakhodaCoord = levelCoords[NakhleNakhodaLevel];
                // if the current level is past the placed entry, we have found a good empty space and can stop.
                // if strictOrder, keep finding more lateral intersections.
                if (!strictOrder && NakhleNakhodaCoord >= candidateCoord + newEntry.thickness) {
                    break;
                }
                var NakhleNakhodaEntries = entriesByLevel[NakhleNakhodaLevel];
                var NakhleNakhodaEntry = void 0;
                var searchRes = binarySearch(NakhleNakhodaEntries, newEntry.span.start, getEntrySpanEnd); // find first entry after newEntry's end
                var lateralIndex = searchRes[0] + searchRes[1]; // if exact match (which doesn't collide), go to next one
                while ( // loop through entries that horizontally intersect
                (NakhleNakhodaEntry = NakhleNakhodaEntries[lateralIndex]) && // but not past the whole entry list
                    NakhleNakhodaEntry.span.start < newEntry.span.end // and not entirely past newEntry
                ) {
                    var NakhleNakhodaEntryBottom = NakhleNakhodaCoord + NakhleNakhodaEntry.thickness;
                    // intersects into the top of the candidate?
                    if (NakhleNakhodaEntryBottom > candidateCoord) {
                        candidateCoord = NakhleNakhodaEntryBottom;
                        touchingEntry = NakhleNakhodaEntry;
                        touchingLevel = NakhleNakhodaLevel;
                        touchingLateral = lateralIndex;
                    }
                    // butts up against top of candidate? (will happen if just intersected as well)
                    if (NakhleNakhodaEntryBottom === candidateCoord) {
                        // accumulate the highest possible stackCnt of the NakhleNakhodaEntries that butt up
                        stackCnt = Math.max(stackCnt, stackCnts[buildEntryKey(NakhleNakhodaEntry)] + 1);
                    }
                    lateralIndex += 1;
                }
            }
